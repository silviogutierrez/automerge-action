...
async function merge(context, pullRequest) {
  ...
  const {commitTitle, commitMessage} = getCommitMessage(mergeCommitMessage, pullRequest);
...
function tryMerge(
  ...
  return retry(
    ...
    mergeRetrySleep,
    () =>
      mergePullRequest(octokit, pullRequest, head, mergeMethod, commitTitle, commitMessage),
    async () => {
      const pr = await getPullRequest(octokit, pullRequest);
      if (pr.merged === true) {
        return "success";
...
function getCommitMessage(mergeCommitMessage, pullRequest) {
  if (mergeCommitMessage === "automatic") {
    return {commitTitle: undefined, commitMessage: undefined};
  } else if (mergeCommitMessage === "pull-request-title") {
    return {commitTitle: pullRequest.title, commitMessage: ""};
  } else if (mergeCommitMessage === "pull-request-description") {
    return {commitTitle: "", commitMessage: pullRequest.body};
  } else if (mergeCommitMessage === "pull-request-title-and-description") {
    return {commitTitle: pullRequest.title, commitMessage: pullRequest.body};
  } else {
    ...
    return {commitTitle: "", commitMessage: mergeCommitMessage};
  }
}
