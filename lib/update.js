const { logger, tmpdir } = require("./common");
...
async function update(context, pullRequest) {
  ...
  if (updateMethod === "merge") {
    newSha = await merge(octokit, pullRequest);
...
async function merge(octokit, pullRequest) {
  const mergeableState = await pullRequestState(octokit, pullRequest);
...
async function pullRequestState(octokit, pullRequest) {
  if (pullRequest.mergeable == null || !pullRequest.mergeable_state || pullRequest.mergeable_state === "unknown") {
